...
_.mixin({
	...
	formatLength : function (y, z) {
		if (y <= 0)
			return "";
		if (z) {
			var t = _.floor(y);
			z = "." + (y - t).toFixed(z).split(".")[1];
		} else {
			var t = _.round(y);
			z = "";
		}
		var w = _.floor(t / 604800);
		var d = _.floor((t -= w * 604800) / 86400);
		var h = _.floor((t -= d * 86400) / 3600);
		var m = _.floor((t -= h * 3600) / 60);
		var s = t -= m * 60;
		var temp = "";
		if (w > 0)
			temp += w + "wk ";
